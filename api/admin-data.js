...
module.exports = async (req, res) => {
  ...
  try {
    const [orders, templates, coupons, customers, referrals, referralOptins, logs, products] = await Promise.all([
      ...
      sql`SELECT * FROM referral_optins ORDER BY created_at DESC`,
    ...
    return res.status(200).json({ success: true, orders, templates, coupons, customers, referrals, referralOptins, logs, products });
  } catch (error) {
