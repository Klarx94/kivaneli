...
async function computeAuthoritativeOrder(cartItems, couponCode) {
  ...
  for (const item of cartItems) {
    ...
    const [product] = await sql`
      SELECT slug, price, impulse_price FROM products WHERE slug = ${item.id} AND is_active = true AND in_stock = true
    `;
    if (!product) throw new Error(`Producto no disponible o agotado: ${item.id}`);
