...
// Cancels an order both at Dropea (if it was ever dispatched there) and in our own DB.
async function cancelDropeaOrder(orderNumber) {
  const [order] = await sql`SELECT * FROM orders WHERE order_number = ${orderNumber}`;
  if (!order) throw new Error(`Order ${orderNumber} not found`);

  let dropeaCancelResult = null;
  if (order.dropea_order_id) {
    dropeaCancelResult = await new Promise((resolve) => {
      const req = https.request({
        hostname: 'es.public-api.dropea.com',
        path: `/dropshipper/orders/${order.dropea_order_id}/cancel`,
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${DROPEA_API_TOKEN}`,
          'Content-Type': 'application/json',
  ...
  await sql`
    UPDATE orders SET shipping_status = 'CANCELLED', updated_at = now()
    WHERE order_number = ${orderNumber}
  `;

  return dropeaCancelResult;
}

// Reads current stock per linked Dropea product. Dropea has no stock-change webhook, so
// this is polled (see the check_stock_alerts cron action in api/admin-actions.js) instead
// of pushed — the only way to catch a silent stock-out before a customer orders it.
function getShopStockLevels() {
  return new Promise((resolve) => {
    const req = https.request({
      hostname: 'es.public-api.dropea.com',
      path: `/dropshipper/shops/${DROPEA_STORE_ID}/products`,
      method: 'GET',
      headers: { 'Authorization': `Bearer ${DROPEA_API_TOKEN}`, 'Accept': 'application/json' },
      timeout: 8000
    }, (res) => {
      let d = '';
      res.on('data', c => d += c);
      res.on('end', () => {
        try {
          ...
          resolve((parsed && parsed.data && parsed.data.items) || []);
        } catch (e) { resolve([]); }
      });
    });
...
module.exports = { resolveDropeaLineItems, sendOrderToDropeaPublicApi, dispatchOrderToDropea, cancelDropeaOrder, getShopStockLevels };
